...
class ExtStringFunctions {
	...
	//Jie Bao add the following
	
	/**
	 * {{#pre:value}}
	 */
	static function runPre( &$parser, $inStr = '' ) {
		return "<pre style=\"white-space: pre-wrap; white-space: -moz-pre-wrap; word-wrap: break-word\">". $inStr ."</pre>";
	}	
	
	/**
	 * {{#code:parameters|code}}
	 * This function requires SyntaxHighlight_GeSHi
	 * See http://www.mediawiki.org/wiki/Extension:SyntaxHighlight_GeSHi
	
	static function runCode( &$parser, $inStr = '', $para='') {
		return "<syntaxhighlight $para>". $inStr ."</syntaxhighlight>";
	}	
	 */
