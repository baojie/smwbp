...
class LdapAuthenticationPlugin extends AuthPlugin {
	...
	function modifyUITemplate( &$template ) {
		...
		if ( !isset( $wgLDAPAddLDAPUsers[$_SESSION['wsDomain']] ) || !$wgLDAPAddLDAPUsers[$_SESSION['wsDomain']] ) {
			...
			$template->Set( 'canreset', true );  //Li Ding: allow reset password via email if create account is allowed
		}

		$template->set( 'usedomain', true );
		//$template->set( 'useemail', false );  //Li Ding: remove this line 
		...
		//Li Ding: Move the selected domain to the top of domain array		
		$selectedDomain=$_SESSION['wsDomain'];
		if ($this->validDomain($selectedDomain)){
			$this->printDebug( "Move the selected domain to the top of domain array.", NONSENSITIVE );
			$newDomArr = array();
			array_push( $newDomArr, $selectedDomain );
			foreach ( $tempDomArr as $other ) {
				if ( strcmp( $other , $selectedDomain ) != 0 ) {
					array_push( $newDomArr, $other );
				}
			}
			$tempDomArr = $newDomArr;
		}	
	...
	function setDomain( $domain ) {
		global $wgLDAPDefaultDomain;

		//Li Ding: Determine the selected domain
		if ($this->validDomain( $_SESSION['wsDomain'])){
			$this->printDebug( "Setting domain as: $domain", NONSENSITIVE );
			$_SESSION['wsDomain'] = $domain;
		}else if ( isset($wgLDAPDefaultDomain) && $this->validDomain($wgLDAPDefaultDomain) ){
...
$wgExtensionCredits['other'][] = array(
	...
	'version' => '1.1g-fix2',
	'author' => 'Ryan Lane',
	'description' => 'LDAP Authentication plugin with support for multiple LDAP authentication methods (fix on domain list from 1.1g)',
